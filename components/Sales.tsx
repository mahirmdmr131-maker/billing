...
const Sales: React.FC<SalesProps> = ({ data, updateData, onNavigateToInvoices }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const totalAmount = finalItems.reduce((sum, i) => sum + i.total, 0);
    ...
    updateData(prev => {
      ...
      // 2. Update Customer Balance if Pending
      let updatedCustomers = [...prev.customers];
      if (formData.paymentMethod === 'Pending' && formData.customerId) {
        updatedCustomers = updatedCustomers.map(c => 
          c.id === formData.customerId ? { ...c, pendingBalance: (c.pendingBalance || 0) + totalAmount } : c
        );
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                {formData.paymentMethod === 'Pending' && !formData.customerId && (
                  <p className="text-[10px] text-red-500 font-bold mt-2 uppercase tracking-tight">Select a registered customer for Credit sales.</p>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-slate-100 gap-6">
              <div className="bg-slate-900 text-white px-8 py-4 rounded-2xl shadow-xl">
                <p className="text-[10px] uppercase font-black opacity-50 tracking-widest mb-1">Grand Total</p>
                <p className="text-3xl font-black">₹{formData.items.reduce((sum, i) => sum + (Number(i.quantity || 0) * Number(i.rate || 0)), 0).toLocaleString()}</p>
              </div>
              <div className="flex items-center space-x-4">
                ...
                <button 
                  ...
                  className="px-12 py-4 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-2xl shadow-lg shadow-emerald-100 transition-all active:scale-95"
                >
                  Save & Print Invoice
                </button>
              </div>
                ...
                <tr key={sale.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-8 py-5 text-center">
                    <div className="flex items-center justify-center space-x-2">
                      ...
                      {sale.isMistake ? (
                        <span className="text-[10px] font-black uppercase bg-red-50 text-red-600 px-3 py-2 rounded-xl border border-red-100">Mistaken</span>
                      ) : (
                        <span className="text-[10px] font-black uppercase bg-emerald-50 text-emerald-600 px-3 py-2 rounded-xl border border-emerald-100">Success</span>
                      )}
                    </div>
                  </td>
                </tr>
