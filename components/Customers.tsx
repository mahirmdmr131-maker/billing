...
const Customers: React.FC<CustomersProps> = ({ data, updateData }) => {
  ...
  const handleRecordPayment = (e: React.FormEvent) => {
    ...
    // Update the local view reference
  ...
  if (viewCustomer) {
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        ...
        <div className="bg-indigo-900 rounded-3xl p-8 text-white shadow-xl flex flex-col md:flex-row justify-between gap-6 relative overflow-hidden">
          <div className="relative z-10">
            <h2 className="text-3xl font-black mb-2">{viewCustomer.name}</h2>
            <div className="flex flex-wrap gap-4 text-sm font-medium opacity-80">
              ...
              {viewCustomer.gst && <span className="px-2 py-0.5 bg-white/10 rounded text-[10px] uppercase">GST: {viewCustomer.gst}</span>}
            </div>
            <p className="mt-4 text-xs opacity-60 max-w-md">{viewCustomer.address || 'No address provided'}</p>
          </div>
          <div className="text-right relative z-10 flex flex-col justify-center">
            <div className="bg-white/10 p-4 rounded-2xl border border-white/10 backdrop-blur-sm">
                <p className="text-[10px] uppercase font-black opacity-50 tracking-[0.2em] mb-1">Current Balance Due</p>
                <p className={`text-5xl font-black ${viewCustomer.pendingBalance > 0 ? 'text-red-400' : 'text-emerald-400'}`}>₹{viewCustomer.pendingBalance.toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 space-y-6">
                <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
                    ...
                    <form onSubmit={handleRecordPayment} className="space-y-4">
                        ...
                        <p className="text-[10px] text-slate-400 text-center italic">This reduces the pending balance for the customer.</p>
                    </form>
                </div>

                <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
                    <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-4">LTV Stats</h4>
                    <div className="space-y-4">
                        <div className="flex justify-between items-center py-3 border-b border-slate-50">
                            <span className="text-xs font-bold text-slate-500 uppercase">Lifetime Sales</span>
                            <span className="font-black text-slate-800">₹{totalSpent.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center py-3">
                            <span className="text-xs font-bold text-slate-500 uppercase">Total Invoices</span>
                            <span className="font-black text-slate-800">{customerSales.length}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="lg:col-span-2 bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="px-8 py-4 bg-slate-50 border-b flex justify-between items-center">
                    <h4 className="font-black text-slate-800 uppercase text-xs tracking-widest">Transaction History</h4>
                </div>
                <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        ...
                        <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                            <td className="px-8 py-4 font-medium text-slate-600">{new Date(s.date).toLocaleDateString()}</td>
                            <td className="px-8 py-4"><span className="font-mono font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded">{s.invoiceNumber}</span></td>
                        ...
                        <tr><td colSpan={4} className="px-8 py-20 text-center text-slate-400 font-medium">No sales recorded for this customer.</td></tr>
                        )}
                    </tbody>
                    </table>
                </div>
